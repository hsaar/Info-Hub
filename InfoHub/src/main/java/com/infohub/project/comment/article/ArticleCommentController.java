...
@Controller
@RequestMapping("/mypage/articleComments")
public class ArticleCommentController {
    ...
    public String listMyComments(@RequestParam("loginNo") int loginNo, Model model) {
        try {
            ...
            return "mypage/articleCommentList"; // articleCommentList.jsp
    ...
    // 내가 쓴 댓글 수정
    @PostMapping("/update")
    public String updateMyComment(@ModelAttribute ArticleCommentDTO dto, Model model) {
        try {
            service.updateMyComment(dto);
            // 수정 후 다시 내가 쓴 댓글 목록으로 이동
            return "redirect:/mypage/articleComments?loginNo=" + dto.getLoginNo();
        } catch (SQLException e) {
            model.addAttribute("error", "DB 오류 발생");
            return "error";
        }
    ...
    public String deleteMyComment(@RequestParam("commentId") int commentId,
        ...
        try {
            ...
            // 삭제 후 다시 내가 쓴 댓글 목록으로 이동
            return "redirect:/mypage/articleComments?loginNo=" + loginNo;
        } catch (SQLException e) {
            model.addAttribute("error", "DB 오류 발생");
            return "error";
        }
