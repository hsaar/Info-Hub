...
<!-- 네비게이션 -->
    ...
    <nav class="news-nav">
      ...
      <a href="articleListAll6" class="active">금융/경제</a>
    </nav>
	</div>
  ...
  <div class="news-container">
	...
	총 <c:out value="${categoryTotalCount}"/>건
      ...
      <c:forEach var="article" items="${articleListAll6}"> <!-- JSTL의 반복문 -->
   ...
   <!-- 페이지네이션 -->
    ...
    <c:if test="${pageMaker.totalCount > 0}">
        <nav class="pagination-nav">
         ...
         <a href="articleListAll6" class="page-number" style="min-width: 80px; font-weight: 700; background: var(--mist-100); border-color: var(--mist-300);"> 처음목록 </a>
            
            <c:if test="${pageMaker.prev}">
                <a href="articleListAll6${pageMaker.makeSearch(pageMaker.startPage-1)}"  class="page-arrow" aria-label="Previous">&lt;</a>
            </c:if>

            <c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
                <a href="articleListAll6?page=${idx }" class="page-number ${pageMaker.cri.page == idx ? 'active' : ''}">${idx }</a>
            </c:forEach>

            <c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
                <a href="articleListAll6${pageMaker.makeSearch(pageMaker.endPage +1) }" class="page-arrow" aria-label="Next">&gt;</a>
            </c:if>
            </nav>
    </c:if>
	</div>
